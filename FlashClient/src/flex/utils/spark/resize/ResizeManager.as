...
//					}
		...
		private function resizeMouseMoveHandler(event:MouseEvent):void {
			...
			var newWidth:Number = startWidth + event.stageX - resizeInitX;
			var newHeight:Number = startHeight + event.stageY - resizeInitY;
//			trace("Changing size from " + oldWidth + "x" + oldHeight + " to " + newWidth + "x" + newHeight);

			var resizeH:Boolean = (resizeDirection != DIRECTION_VERTICAL || resizeDirection == DIRECTION_BOTH);
			var resizeV:Boolean = (resizeDirection != DIRECTION_HORIZONTAL || resizeDirection == DIRECTION_BOTH);

			// restrict the width/height
			newWidth = Math.max(newWidth, resizeComponent.minWidth);
			newWidth = Math.min(newWidth, resizeComponent.maxWidth);
			newHeight = Math.max(newHeight, resizeComponent.minHeight);
			newHeight = Math.min(newHeight, resizeComponent.maxHeight);
